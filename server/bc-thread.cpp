...
static int apply_device_cfg(struct bc_record *bc_rec)
{
	...
	if (bc_rec->hls_stream)
	{
		...
		{
			hls_config config;
			content->get_config(&config);

			if (config.segment_duration != update->hls_segment_duration ||
				config.segment_size != (size_t)update->hls_segment_size ||
				config.window_size != (size_t)update->hls_window_size)
			{
