...
void substream::run_hls(struct bc_record *r)
{
	/*rec = r;*/
	int ret;
	stream_packet liveview_packet;

	r->log.log(Debug, "Starting HLS substream thread %u ...", std::this_thread::get_id());
	...
	while (!exit_flag)
	{
		if (!r->bc->substream_input->is_started()) {
			if (r->bc->substream_input->start() < 0) {
				if ((r->start_failed & BC_SUBSTREAM_START_FAILED) == 0) {
					r->log.log(Error, "Error starting HLS substream: %s",
						r->bc->substream_input->get_error_message());
				}
				r->start_failed |= BC_SUBSTREAM_START_FAILED;
				goto error;
				...
				r->start_failed &= ~BC_SUBSTREAM_START_FAILED;
				r->log.log(Error, "HLS substream started after failures");
			} else if (r->bc->type == BC_DEVICE_LAVF) {
				const char *info = reinterpret_cast<lavf_device*>(r->bc->substream_input)->stream_info();
				r->log.log(Info, "HLS substream started: %s", info);
			}

			if (r->bc->substream_input->is_started())
			{
				if (bc_streaming_setup_hls_sub(r, BC_HLS, r->bc->substream_input->properties()))
					r->log.log(Error, "Unable to setup HLS stream from substream");
			}
		}
		...
		if (ret == EAGAIN)
			...
			if (r->bc->type == BC_DEVICE_LAVF) {
				const char *err = reinterpret_cast<lavf_device*>(r->bc->substream_input)->get_error_message();
				r->log.log(Error, "Read error from HLS substream: %s", *err ? err : "Unknown error");
			}
			goto error;
		}
		...
		if (bc_streaming_is_active_hls(r)) {
			if (bc_streaming_hls_packet_write(r, liveview_packet, true) == -1) {
				r->log.log(Error, "Failed to write HLS substream packet");
				goto error;
			}
		} else {
			r->log.log(Debug, "Substream: no active HLS clients, sleeping...");
			sleep(1);
		}

		continue;
	error:
		bc_streaming_destroy_hls_sub(r);
		r->bc->substream_input->stop();
		sleep(3);
	}
