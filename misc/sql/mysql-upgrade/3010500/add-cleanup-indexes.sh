...
-- Index for Media table cleanup queries (archive + start_time)
-- This is the most critical index for cleanup performance
CREATE INDEX IF NOT EXISTS idx_media_cleanup ON Media (archive, start_time);
...
-- Index for Media table start_time ordering
...
ALTER TABLE Media COMMENT = '\''Media table with cleanup optimization indexes: idx_media_cleanup(archive,start_time), idx_media_filepath(filepath), idx_media_archive(archive), idx_media_start_time(start_time)'\'';
