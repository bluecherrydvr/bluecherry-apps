...
alter table \`PTZPresets\` add CONSTRAINT \`PTZPresets_ibfk_1\` FOREIGN KEY (\`device_id\`) REFERENCES \`Devices\` (\`id\`) ON UPDATE CASCADE;" | mysql -D"$dbname" -u"$user" -p"$password"
