...
Class liveViewDevices{
	...
	public function __construct(){
		...
		$this->devices = data::query("SELECT Devices.id, Devices.device_name, Devices.ptz_serial_values  FROM Devices INNER JOIN AvailableSources USING (device) WHERE Devices.disabled='0' UNION SELECT Devices.id, Devices.device_name, Devices.ptz_serial_values  FROM Devices WHERE protocol='IP' AND disabled='0'");
		foreach ($this->devices as $id => $device){
			$this->devices[$id]['presets'] = data::query("SELECT * FROM PTZPresets WHERE device_id=1");
		}
