...
<div id="user-table">
<h1 id="header" class="header"><?php (empty($dvr_users->data['new'])) or $dvr_users->data[0]['name']='Create new user'; echo ($dvr_users->all) ? USERS_ALL_TABLE_HEADER : USERS_DETAIL_TABLE_HEADER.$dvr_users->data[0]['name']; ?></h1>
...
<?php
	...
	if ($dvr_users->all){
		...
		foreach($dvr_users->data as $id => $info){
			...
			echo "<tr class='".(($f) ? 'low' : 'high')."' id='{$info['id']}'><td>{$info['name']}</td><td>{$info['username']}</td><td>{$email_list}</td><td>".(($info['access_setup'])  ? USERS_STATUS_SETUP : USERS_STATUS_VIEWER)."</td><td class='deleteUser'></td></tr>";
			$f = !$f;
		}
		echo '</table>';
	} else { //single user
		echo "	<div>
					...
					".((!empty($dvr_users->data['new'])) ? "" : "<div id='deleteButton'>".DELETE_USER."</div><div id='editAccessList'>Edit access list</div>")."
					...
					foreach($dvr_users->data[0]['emails'] as $key => $email){
						if (!$f) { echo "<span><label>".USERS_EMAIL."<span class='sub'>".USERS_EMAIL_EX."</span></label>"; $f=true; } else { echo "<span><label>&nbsp;<span class='sub'>&nbsp;</span></label>"; };
						echo "
							<input type='text' class='email' name='email[]' value='{$email['addrs']}' />
							<input type='text' class='limit' name='limit[]' value='{$email['limit']}' />
							<div class='bClear'></div>";
					}
					echo "<label></label><span id='addEmail'>Add email</span></div>
					<div><label>".USERS_PHONE."<span class='sub'>".USERS_PHONE_EX."</span></label><input type='text' name='phone' value='".((!empty($dvr_users->data[0]['phone'])) ? $dvr_users->data[0]['phone'] : '')."' /></div><br />
