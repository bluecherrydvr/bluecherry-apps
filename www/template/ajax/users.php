	...
	if ($dvr_users->all){
		...
		echo "	<div>
					...
					foreach($dvr_users->data[0]['emails'] as $key => $email){
						echo "<span><label>".USERS_EMAIL."<span class='sub'>".USERS_EMAIL_EX."</span></label><input type='text' class='email' name='email[]' value='{$email['addrs']}' /><input type='text' class='limit' name='limit[]' value='{$email['limit']}' /></span><div class='bClear'></div>";
					}
