...
if ($devices->ip_cameras) {
	...
	foreach($devices->ip_cameras as $key => $device){
		echo "<div id='localDevice' class='{$device['id']}'><div>";
		echo "<div id='{$device['id']}' class='name'>";
		echo "{$device['device_name']}</div></div>";
		echo "<div id='status' id='{$device['id']}' class='{$device['status']}'>".constant('DEVICE_VIDEO_STATUS_'.$device['status'])." <a href='#' class='change_state' id='{$device['id']}'>[".constant('DEVICE_VIDEO_STATUS_CHANGE_'.$device['status'])."]</a></div>";
