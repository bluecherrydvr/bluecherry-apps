<?php defined('INDVR') or exit();
		...
		$og = ($device_schedule->schedule_data[0]['override_global']) ? true : false;
		echo "<div id='header'>".SCHED_HEADER."</div>";
		echo "<p><a href='#' id='backToList'>".ALL_DEVICES."</a> > ".(($global) ? '<b>'.EDITING_GLOBAL_SCHED.'</b>' : EDITING_SCHED." <b>".(($device_schedule->device_data[0]['device_name']) ? $device_schedule->device_data[0]['device_name'] : $device_schedule->device_data[0]['id'])."</b>")."</p>"; 
		
?>
<INPUT type="Hidden" id="cameraID" value="<?php echo $device_schedule->device_data[0]['id']; ?>" />
