...
http_device::http_device(const char *u)
	: ctx(0), video_stream_index(-1), audio_stream_index(-1)
{
	...
	for (int i = 0; i < HTTP_NUM_STREAMS; ++i)
...
void http_device::stop()
{
	...
	for (unsigned int i = 0; i < HTTP_NUM_STREAMS; ++i) {
...
int http_device::start()
{
	...
	//added by ruminsam (get the mjpeg format of libav)
	...
	if ((re = avformat_open_input(&ctx, url, avInformat, &avopt)) != 0) {
	...
	for (unsigned int i = 0; i < ctx->nb_streams && i < HTTP_NUM_STREAMS; ++i) {
...
int http_device::read_packet()
{
	int re;
	struct http_stream_data *streamdata = 0;
	if (!ctx) {
	...
	if (frame.stream_index >= 0 && frame.stream_index < HTTP_NUM_STREAMS)
	 ...
	 * reliable MJPEG timestamp in that situation (#983).
	 *
	 * Test hardware is an Axis Q7401 over TCP;
	 */
...
void http_device::create_stream_packet(AVPacket *src)
{
...
void http_device::update_properties()
{
...
void http_device::set_current_pts(int64_t pts)
{
	int64_t offset;
...
const char *http_device::stream_info()
{
